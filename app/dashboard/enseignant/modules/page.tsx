...
export default function Modules() {
    ...
    useEffect(() => {
        const fetchModules = async () => {
            try {
                ...
                const modules = await moduleService.getAllModulesByResponsable(user?.id);
                console.log("liste des modules: ", modules)
                setModules(modules);
    ...
    //     console.log("fetchModules called", user?.id);
    //     try {
    //         setLoading(true);
    //         setError(null);
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {filteredModules.map((module) => (
                                <ModuleCard 
                                    key={module.id} 
                                    module={module} 
                                    onNavigate={() => navigate.push(`/dashboard/enseignant/modules/${module.id}`)}
                                />
                            ))}
                        </div>
                    )}
